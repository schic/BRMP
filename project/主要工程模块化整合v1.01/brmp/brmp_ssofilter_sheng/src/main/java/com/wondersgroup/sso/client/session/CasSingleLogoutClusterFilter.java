package com.wondersgroup.sso.client.session;
...
public class CasSingleLogoutClusterFilter implements Filter {
	...
	public CasSingleLogoutClusterFilter(List<String> peerURLs) {
		try {
		...
		for (String peerDescription : peerURLs) {
			try {
		...
		CasSingleLogoutClusterFilter.LOG.info("SLO cluster peers: [{}]", peerURLs.toString());
	}
